...
bool Gameboard::areLocsEmpty(std::vector<Point> locs) const {
	for (int i = 0; i < locs.size(); i++) {
		if (locs[i].getX() < MAX_X && 
			locs[i].getX() >= 0 && 
			locs[i].getY() < MAX_Y &&
			locs[i].getY() >= 0
			) {
			if (grid[locs[i].getX()][locs[i].getY()] != EMPTY_BLOCK) return false;
		}
	}
	return true;
}
...
int Gameboard::removeCompletedRows() {
	std::vector<int> compRowInd = getCompletedRowIndices();
	removeRows(compRowInd);
	return compRowInd.size();
}
...
void Gameboard::printToConsole() const {
	for (int y = 0; y < MAX_Y; y++) {
		for (int x = 0; x < MAX_X; x++) {
			std::cout << std::setw(2) << grid[x][y] << std::setw(2);
		}
		std::cout << '\n';
	}
}
